...
campaignRouter.delete(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      console.error(error);
      res.status(500).json({
        error: "An error occurred while deleting the email campaigns",
      });
    }
  },
);
...
campaignRouter.delete(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "An error occurred while deleting the SMS campaigns" });
    }
  },
);
...
campaignRouter.delete(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      console.error(error);
      res.status(500).json({
        error: "An error occurred while deleting the WhatsApp campaigns",
      });
    }
  },
);
...
campaignRouter.delete("/email/:id", async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "An error occurred while deleting the email campaign" });
  }
});
...
campaignRouter.delete("/sms/:id", async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "An error occurred while deleting the SMS campaign" });
  }
});
...
campaignRouter.delete("/whatsapp/:id", async (req: Request, res: Response) => {
  try {
    ...
    console.error(error);
    res.status(500).json({
      error: "An error occurred while deleting the WhatsApp campaign",
    });
  }
});
...
campaignRouter.post(
  "/:campaignID/create/emailcamp",
  async (req: Request, res: Response) => {
    const { campaignID } = req.params;
    console.log(req.body);
    const { email, template, scheduled, campaignName, bucket, time } = req.body;
    ...
    try {
      ...
      const emailCampaign = await prisma.emailCampaign.create({
        data: {
          ...
          status: "PENDING",
          regionsClicks: [],
          timeToBeSent: time, // Ensure 'time' is in a proper date format
          bucket: {
            connect: {
...
// Create SMS Campaign
campaignRouter.post(
  "/:campaignID/create/smscamp",
  async (req: Request, res: Response) => {
    const { campaignID } = req.params;
    const { number, template, scheduled, campaignName, bucket, time } =
      req.body;
    ...
    try {
      ...
      console.log(`SMS campaign created: ${smsCampaign}`);
...
campaignRouter.post(
  "/:campaignID/create/whatsappcamp",
  async (req: Request, res: Response) => {
    const { campaignID } = req.params;
